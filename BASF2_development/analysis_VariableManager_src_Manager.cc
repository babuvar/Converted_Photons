...
bool Variable::Manager::createVariable(const std::string& name)
{
  ...
  if (std::regex_match(name, results, std::regex("^([a-zA-Z0-9_]*)\\((.*)\\)$"))) {
    ...
    // Search function name in string parameter variables
    auto string_parameterIter = m_string_parameter_variables.find(functionName);
    if (string_parameterIter!= m_string_parameter_variables.end()) {
	
      std::vector<std::string> arguments;
      for (auto& arg : functionArguments) {
        arguments.push_back(arg);
      }
      auto spfunc = string_parameterIter->second->function;
      auto func = [spfunc, arguments](const Particle * particle) -> double { return spfunc(particle, arguments); };
      m_variables[name] = std::make_shared<Var>(name, func, string_parameterIter->second->description, string_parameterIter->second->group);
      return true;
...
void Variable::Manager::registerVariable(const std::string& name, const Variable::Manager::StringParameterFunctionPtr& f,
                                         const std::string& description)
{
  if (!f) {
    B2FATAL("No function provided for variable '" << name << "'.");
  ...
  auto mapIter = m_string_parameter_variables.find(name);
  if (mapIter == m_string_parameter_variables.end()) {
    auto var = std::make_shared<StringParameterVar>(name, f, description, m_currentGroup);
    std::string rawName = name.substr(0, name.find('('));
    assertValidName(rawName);
    B2DEBUG(19, "Registered string-parameter Variable " << rawName);
    m_string_parameter_variables[rawName] = var;
    m_variablesInRegistrationOrder.push_back(var.get());
  } else {
    B2FATAL("A variable named '" << name << "' was already registered! Note that all variables need a unique name!");
  }
