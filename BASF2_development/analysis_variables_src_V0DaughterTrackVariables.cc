...
namespace Belle2 {
  namespace Variable {
    ...
    int convertedPhotonErrorChecks(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Check that exactly two descendant indices are provided
      if (generalizedDaughterIndices.size() != 2) {
        B2ERROR("Invalid number of daughter indices. Please specify exactly two valid daughter indices.");
        return -1;
      }

      //Load descendants corresponding to generalized indices provided
      auto generalizedIndex1 = generalizedDaughterIndices[0];
      const Particle* dauPart1 = part->getParticleFromGeneralizedIndexString(generalizedIndex1);
      if (!dauPart1) {
        B2ERROR("particle descendant corresponding to index "<<generalizedDaughterIndices[0]<<" does not exist.");
	return -1;
      }
      auto generalizedIndex2 = generalizedDaughterIndices[1];
      const Particle* dauPart2 = part->getParticleFromGeneralizedIndexString(generalizedIndex2);
      if (!dauPart2) {
        B2ERROR("particle descendant corresponding to index "<<generalizedDaughterIndices[1]<<" does not exist.");
	return -1;
      }

      //Check that there exists tracks associated with the daughter indices provided
      if (!dauPart1->getTrack()) {
        B2ERROR("There is no track associated with daughter index " << generalizedDaughterIndices[0]);
        return -1;
      }
      if (!dauPart2->getTrack()) {
        B2ERROR("There is no track associated with daughter index " << generalizedDaughterIndices[1]);
        return -1;
      }

      //Check whether tracks used to calculate variable has been reconstructed as electrons/positrons or not (INCONSEQUENTIAL)
      if (fabs(dauPart1->getPDGCode()) != 11) {
        B2INFO("The first track provided has not been reconstructed as an electron/positron. It has PDG code " << 
                 dauPart1->getPDGCode() << ". However, this is still fully admissible.");
      }
      if (fabs(dauPart2->getPDGCode()) != 11) {
        B2INFO("The second track provided has not been reconstructed as an electron/positron. It has PDG code " << 
                 dauPart2->getPDGCode() << ".However, this is still fully admissible.");
      }
    ...
    int convertedPhotonLoadHelixParams(const Particle* dauPart1, const Particle* dauPart2, double& Phi01, double& D01,
                                       double& Omega1, double& Z01, double& TanLambda1, double& Phi02, double& D02, double& Omega2, double& Z02,
                                       double& TanLambda2)
    {
      //Get helix parameters
      //Electron/track 1
      Helix e1Helix = dauPart1->getTrackFitResult()->getHelix();
      ...
      //Electron/track 2
      Helix e2Helix = dauPart2->getTrackFitResult()->getHelix();
    ...
    double convertedPhotonInvariantMass(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
      ...
      auto generalizedIndex2 = generalizedDaughterIndices[1];
      const Particle* dauPart2 = part->getParticleFromGeneralizedIndexString(generalizedIndex2);
      convertedPhotonLoadHelixParams(dauPart1, dauPart2, Phi01, D01, Omega1, Z01, TanLambda1, Phi02, D02,
    ...
    double convertedPhotonDelTanLambda(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
      if (errFlag == -1) {return std::numeric_limits<double>::quiet_NaN();}
    ...
    double convertedPhotonDelR(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
      if (errFlag == -1) {return std::numeric_limits<double>::quiet_NaN();}
      ...
      auto generalizedIndex2 = generalizedDaughterIndices[1];
      const Particle* dauPart2 = part->getParticleFromGeneralizedIndexString(generalizedIndex2);
      errFlag = convertedPhotonLoadHelixParams(dauPart1, dauPart2, Phi01, D01, Omega1, Z01, TanLambda1, Phi02, D02,
                                     Omega2, Z02, TanLambda2);
      if (errFlag == -1) {
    ...
    std::pair<double, double> convertedPhotonZ1Z2(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
    ...
    double convertedPhotonDelZ(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      std::pair<double, double> z1z2 = convertedPhotonZ1Z2(part, generalizedDaughterIndices);
      double z1 = z1z2.first; double z2 = z1z2.second;
      return (z1 - z2);
    }

    double convertedPhotonZ(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      std::pair<double, double> z1z2 = convertedPhotonZ1Z2(part, generalizedDaughterIndices);
      double z1 = z1z2.first; double z2 = z1z2.second;
      return (z1 + z2) * 0.5;
    ...
    TVector2 convertedPhotonXY(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
    ...
    double convertedPhotonX(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto vtxXY = convertedPhotonXY(part, generalizedDaughterIndices);
      return vtxXY.X();
    }

    double convertedPhotonY(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto vtxXY = convertedPhotonXY(part, generalizedDaughterIndices);
      return vtxXY.Y();
    }

    double convertedPhotonRho(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto vtxXY = convertedPhotonXY(part, generalizedDaughterIndices);
      return vtxXY.Mod();
    }

    TVector3 convertedPhoton3Momentum(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      //Do basic checks
      int errFlag = convertedPhotonErrorChecks(part, generalizedDaughterIndices);
      ...
      double p2  = dauPart1->getMomentumMagnitude();
    ...
    double convertedPhotonPx(const Particle* part,  const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto gammaMomentum = convertedPhoton3Momentum(part, generalizedDaughterIndices);
      return gammaMomentum.Px();
    }

    double convertedPhotonPy(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto gammaMomentum = convertedPhoton3Momentum(part, generalizedDaughterIndices);
      return gammaMomentum.Py();
    }

    double convertedPhotonPz(const Particle* part, const std::vector<std::string>& generalizedDaughterIndices)
    {
      auto gammaMomentum = convertedPhoton3Momentum(part, generalizedDaughterIndices);
    ...
    REGISTER_VARIABLE("convertedPhotonInvariantMass(i,j)",       convertedPhotonInvariantMass,
                      "Invariant mass of the i-j daughter system as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonDelTanLambda(i,j)",       convertedPhotonDelTanLambda,
                      "Discriminating variable Delta-TanLambda calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonDelR(i,j)",       convertedPhotonDelR,
                      "Discriminating variable Delta-R calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonDelZ(i,j)",       convertedPhotonDelZ,
                      "Discriminating variable Delta-Z calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonX(i,j)",       convertedPhotonX,
                      "Estimate of vertex X coordinate  calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonY(i,j)",       convertedPhotonY,
                      "Estimate of vertex Y coordinate  calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonZ(i,j)",       convertedPhotonZ,
                      "Estimate of vertex Z coordinate  calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonRho(i,j)",       convertedPhotonRho,
                      "Estimate of vertex Rho  calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonPx(i,j)", convertedPhotonPx,
                      "Estimate of x-component of photon momentum calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonPy(i,j)", convertedPhotonPy,
                      "Estimate of y-component of photon momentum calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
    REGISTER_VARIABLE("convertedPhotonPz(i,j)", convertedPhotonPz,
                      "Estimate of z-component of photon momentum calculated for daughters (i,j) as defined in https://indico.belle2.org/event/3644/contributions/18622/attachments/9401/14443/Photon_vertexin_B2GM.pdf, assuming it's a converted photon. Note that i and j are generalized indices with colon separation indicating generation of daughter.");
